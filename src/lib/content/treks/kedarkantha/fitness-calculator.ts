...
export const FITNESS_LEVELS: FitnessLevel[] = [
  {
    ...
    scoreMax: 39,
    summary:
      'Your current base needs structured build-up before Kedarkantha. Start training at least 8–10 weeks before departure.',
    weeksRecommended: '8–10 weeks',
    readiness:
      'Focus on building daily walking habit, basic cardio, and leg strength before attempting long snow walks at altitude.',
    weeklyPlan: [
    ...
    warnings: [
      ...
      'Do not skip the gradual build-up — altitude and snow will feel harder than plain-ground walks.',
    ],
  },
  {
    id: 'building-base',
    ...
    scoreMax: 59,
    summary:
      'You have a reasonable foundation. A focused 6–8 week plan will bring you to a comfortable Kedarkantha fitness level.',
    weeksRecommended: '6–8 weeks',
    readiness:
      'Increase endurance and backpack comfort; add hill repeats and longer weekend sessions.',
    weeklyPlan: [
  ...
  },
  {
    ...
    scoreMax: 79,
    summary:
      'You are close to the fitness level most Kedarkantha trekkers need. Refine endurance and altitude habits over 4–6 weeks.',
    weeksRecommended: '4–6 weeks',
    readiness:
      'Maintain cardio, add snow-terrain simulation where possible, and prioritise recovery and hydration.',
    weeklyPlan: [
    ...
    warnings: [
      ...
      'Report any persistent breathlessness to your trek leader on Day 1 drive.',
    ],
  },
  {
    id: 'strong-ready',
    ...
    scoreMax: 100,
    summary:
      'Your fitness profile is strong for Kedarkantha. Maintain conditioning and focus on altitude-specific habits.',
    weeksRecommended: '2–4 weeks',
    readiness:
      'Keep weekly cardio and one long hike; emphasise recovery, sleep, and hydration before travel.',
    weeklyPlan: [
    ...
    warnings: [
      'Even strong trekkers can feel altitude — pace yourself on summit day.',
      'Avoid alcohol and heavy meals the night before the summit push.',
    ],
  },
];
...
export function assessKedarkanthaFitness(input: FitnessAssessmentInput): FitnessAssessmentResult {
  ...
  const level =
    FITNESS_LEVELS.find((l) => score >= l.scoreMin && score <= l.scoreMax) ??
    FITNESS_LEVELS[0];
  ...
  if (input.priorHighAltitude === 'never') tips.push('Spend a day acclimatising at Sankri before the trek starts.');
  if (input.weeklyHiking === 'rarely') tips.push('Schedule one long outdoor walk every weekend with your trek backpack.');
  if (input.exerciseDays === '0-1') tips.push('Move to at least 4 active days per week — consistency matters more than intensity early on.');
  if (!tips.length) tips.push('Maintain your current routine and prioritise sleep, hydration, and a gradual taper before departure.');
...
export function fitnessLevelForScore(score: number): FitnessLevel {
  return (
    FITNESS_LEVELS.find((l) => score >= l.scoreMin && score <= l.scoreMax) ?? FITNESS_LEVELS[0]
  );
}
