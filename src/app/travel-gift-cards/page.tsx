      ...
      <div className="container mx-auto max-w-4xl px-4">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
              {[
                ...
                <div key={f.t} className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
                  <div className="w-10 h-10 bg-[#ffaf21]/10 rounded-xl flex items-center justify-center mx-auto mb-3"><f.icon className="w-5 h-5 text-[#ffaf21]" /></div>
                  <h3 className="font-bold text-sm text-gray-900 mb-1">{f.t}</h3>
                  <p className="text-xs text-gray-500">{f.d}</p>
                </div>
              ))}
            </div>
            <button onClick={() => setStep(2)} className="bg-[#ffaf21] hover:bg-[#d49400] text-gray-900 font-semibold px-8 py-3.5 rounded-full transition-all inline-flex items-center gap-2">
        ...
        {step === 2 && (
          <form onSubmit={handlePurchase} className="max-w-2xl mx-auto">
            <button onClick={() => setStep(1)} className="text-sm text-gray-500 hover:text-[#ffaf21] mb-6">&larr; Back</button>
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 lg:p-8 space-y-6">
                ...
                <div className="grid grid-cols-3 lg:grid-cols-6 gap-3">
                  {amounts.map(a => (
                    <button key={a} type="button" onClick={() => setForm(f=>({...f,amount:a}))}
                      className={`py-3 rounded-xl border-2 text-sm font-semibold transition-all ${form.amount === a ? 'border-[#ffaf21] bg-[#ffaf21]/5 text-[#ffaf21]' : 'border-gray-100 text-gray-600 hover:border-gray-200'}`}>
                      ₹{a.toLocaleString()}
                    </button>
                  ))}
                </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Your Name *</label><input type="text" required value={form.senderName} onChange={e => setForm(f=>({...f,senderName:e.target.value}))} className="w-full px-4 py-2.5 rounded-xl border border-gray-200 outline-none focus:border-[#ffaf21] text-sm" /></div>
              </div>
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Message (optional)</label><textarea rows={3} value={form.message} onChange={e => setForm(f=>({...f,message:e.target.value}))} className="w-full px-4 py-2.5 rounded-xl border border-gray-200 outline-none focus:border-[#ffaf21] text-sm resize-none" placeholder="Write a heartfelt message..." /></div>
              ...
              <div className="bg-gradient-to-r from-pink-50 to-purple-50 rounded-xl p-5 border border-pink-100">
                ...
                <div className="bg-white rounded-xl p-4 border border-gray-100">
                  <div className="text-center">
                    <div className="text-2xl font-bold text-[#ffaf21]">₹{form.amount.toLocaleString()}</div>
              ...
              <button type="submit" className="w-full bg-[#ffaf21] hover:bg-[#d49400] text-gray-900 font-semibold px-8 py-3.5 rounded-full transition-all text-sm">
        ...
        {step === 3 && (
          <div className="max-w-lg mx-auto text-center">
            <div className="w-16 h-16 bg-[#ffaf21]/10 rounded-full flex items-center justify-center mx-auto mb-4">
              <Check className="w-8 h-8 text-[#ffaf21]" />
            </div>
            ...
            <div className="bg-gray-50 rounded-2xl p-6 mb-6">
              ...
              <div className="flex items-center justify-center gap-2">
                <span className="font-mono text-xl lg:text-2xl font-bold text-[#ffaf21]">{code}</span>
                <button onClick={() => navigator.clipboard.writeText(code)} className="p-1.5 text-gray-400 hover:text-[#ffaf21]">
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <button onClick={() => { setStep(1); setCode(''); setForm({ amount: 5000, recipientName: '', recipientEmail: '', message: '', senderName: '' }); }}
                className="bg-[#ffaf21] text-gray-900 font-semibold px-8 py-3 rounded-full text-sm hover:bg-[#d49400]">Buy Another</button>
