...
import { ArrowRight, Shield, Check, ChevronRight, Star, Clock, Users, Phone, Calendar, CreditCard, Lock, Wallet, Percent, Gift, Loader } from 'lucide-react';
...
function BookingContent() {
  ...
  const [form, setForm] = useState({
    name: '', email: '', phone: '', persons: '1', date: '',
  ...
  if (!trek) return (
    <div className="pt-28 min-h-screen flex items-center justify-center">
      <div className="text-center">
        ...
        <Link href="/treks" className="inline-flex items-center gap-2 bg-[#ffaf21] hover:bg-[#d49400] text-gray-900 font-semibold px-6 py-3 rounded-full transition-all">Browse All Treks <ArrowRight className="w-4 h-4" /></Link>
      </div>
    </div>
  );
    ...
    <div className="pt-20 lg:pt-24 pb-12 lg:pb-20 bg-gray-50 min-h-screen">
      <div className="container mx-auto max-w-5xl">
        <div className="mb-6 lg:mb-8">
          <Link href={`/treks/${trek.id}`} className="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-[#ffaf21] transition-colors mb-4">
        ...
        <div className="flex items-center justify-center gap-1 lg:gap-3 mb-8 lg:mb-10">
          {steps.map((s, i) => (
            <div key={s.num} className="flex items-center gap-1 lg:gap-3">
              <div className={`flex items-center gap-2 lg:gap-3 px-3 lg:px-4 py-2 rounded-full transition-all ${step >= s.num ? 'bg-[#ffaf21] text-gray-900 shadow-sm shadow-[#ffaf21]/30' : 'bg-white text-gray-400 border border-gray-200'}`}>
                <div className={`w-6 h-6 lg:w-7 lg:h-7 rounded-full flex items-center justify-center text-xs lg:text-sm font-bold ${step > s.num ? 'bg-white text-[#ffaf21]' : ''}`}>
              ...
              {i < steps.length - 1 && <div className={`w-6 lg:w-10 h-0.5 ${step > s.num ? 'bg-[#ffaf21]' : 'bg-gray-200'}`} />}
            </div>
          ))}
        </div>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
          <div className="lg:col-span-2 space-y-5">
              ...
              <>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    {trek.pricing.map(p => (
                      <button key={p.name} type="button" onClick={() => setForm(f => ({ ...f, pkg: p.name }))}
                        className={`relative p-4 rounded-xl border-2 text-center transition-all ${form.pkg === p.name ? 'border-[#ffaf21] bg-[#ffaf21]/5 shadow-sm' : 'border-gray-100 hover:border-gray-200 bg-white'}`}>
                        {p.badge && <span className="absolute -top-2.5 left-1/2 -translate-x-1/2 bg-[#ffaf21] text-gray-900 text-[10px] font-bold px-3 py-1 rounded-full whitespace-nowrap">{p.badge}</span>}
                        <div className="mt-1">
                          <div className="font-bold text-sm text-[#000000]">{p.name}</div>
                          <div className="text-xl lg:text-2xl font-bold text-[#ffaf21] mt-1">₹{p.price.toLocaleString()}</div>
                          ...
                          <div className="mt-3 pt-3 border-t border-gray-100">
                            <div className="text-xs font-semibold text-[#ffaf21]">Deposit ₹{p.deposit.toLocaleString()}</div>
                          </div>
                        </div>
                      </button>
                    ))}
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input type="date" required value={form.date} onChange={e => setForm(f => ({ ...f, date: e.target.value }))}
                          className="w-full pl-10 pr-4 py-2.5 rounded-lg border border-gray-200 focus:border-[#ffaf21] focus:ring-2 focus:ring-[#ffaf21]/20 outline-none transition-all text-sm" />
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <select value={form.persons} onChange={e => setForm(f => ({ ...f, persons: e.target.value }))}
                          className="w-full pl-10 pr-4 py-2.5 rounded-lg border border-gray-200 focus:border-[#ffaf21] focus:ring-2 focus:ring-[#ffaf21]/20 outline-none transition-all text-sm appearance-none bg-white">
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 lg:p-7">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    {[
                      ...
                      <button key={o.v} type="button" onClick={() => setForm(f => ({ ...f, payment: o.v }))}
                        className={`p-4 rounded-xl border-2 text-center transition-all ${form.payment === o.v ? 'border-[#ffaf21] bg-[#ffaf21]/5 shadow-sm' : 'border-gray-100 hover:border-gray-200'}`}>
                  ...
                  <div className="mt-4 flex items-center gap-2 text-xs text-gray-500 bg-gray-50 rounded-lg p-3">
                    <Lock className="w-3.5 h-3.5 text-[#ffaf21]" /> Secure payment via WhatsApp. No card details stored.
                  </div>
                </div>
              </>
            )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div>
                    ...
                    <input type="text" required value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                      className="w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:border-[#ffaf21] focus:ring-2 focus:ring-[#ffaf21]/20 outline-none transition-all text-sm" placeholder="Your full name" />
                  </div>
                  <div>
                    ...
                    <input type="email" required value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))}
                      className="w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:border-[#ffaf21] focus:ring-2 focus:ring-[#ffaf21]/20 outline-none transition-all text-sm" placeholder="email@example.com" />
                  </div>
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="tel" required value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))}
                        className="w-full pl-10 pr-4 py-2.5 rounded-lg border border-gray-200 focus:border-[#ffaf21] focus:ring-2 focus:ring-[#ffaf21]/20 outline-none transition-all text-sm" placeholder="+91 98765 43210" />
                    </div>
                  </div>
                  <div>
                    ...
                    <input type="text" value={form.notes.split(',')[0] || ''} onChange={e => setForm(f => ({ ...f, notes: e.target.value + (f.notes.includes(',') ? f.notes.substring(f.notes.indexOf(',')) : '') }))}
                      className="w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:border-[#ffaf21] focus:ring-2 focus:ring-[#ffaf21]/20 outline-none transition-all text-sm" placeholder="Your city" />
                  </div>
                </div>
                <div className="mt-5">
                  ...
                  <textarea rows={2} value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))}
                    className="w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:border-[#ffaf21] focus:ring-2 focus:ring-[#ffaf21]/20 outline-none transition-all text-sm resize-none" placeholder="Dietary needs, health conditions, room preferences..." />
                </div>
                <div className="mt-5 bg-[#ffaf21]/5 rounded-xl p-4 flex items-start gap-3">
                  <Shield className="w-5 h-5 text-[#ffaf21] shrink-0 mt-0.5" />
            ...
            {step === 3 && (
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 lg:p-7">
                <div className="text-center mb-6">
                  <div className="w-14 h-14 bg-[#ffaf21]/10 rounded-full flex items-center justify-center mx-auto mb-3"><Check className="w-7 h-7 text-[#ffaf21]" /></div>
                ...
                <div className="bg-gray-50 rounded-xl p-5 space-y-3">
                  ...
                  <div className="flex justify-between items-center"><span className="text-gray-500 text-sm">Payable Now</span><span className="font-bold text-lg text-[#ffaf21]">₹{payableNow.toLocaleString()}</span></div>
                </div>
              </div>
            )}
            ...
            <div className="flex gap-3">
              ...
              <button type="submit" className={`flex-1 flex items-center justify-center gap-2 font-semibold px-6 py-3 rounded-full transition-all text-sm shadow-sm ${step === 3 ? 'bg-[#ffaf21] hover:bg-[#d49400] text-gray-900 shadow-[#ffaf21]/25' : 'bg-[#ffaf21] hover:bg-[#d49400] text-gray-900 shadow-[#ffaf21]/25'}`}>
          ...
          <div className="lg:col-span-1">
            <div className="lg:sticky lg:top-24 space-y-5">
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                ...
                <div className="p-5 space-y-3 text-sm">
                  <div className="flex items-center gap-3 text-gray-600">
                    <Clock className="w-4 h-4 text-[#ffaf21]" /><span>{trek.duration}</span>
                  </div>
                  <div className="flex items-center gap-3 text-gray-600">
                    <Users className="w-4 h-4 text-[#ffaf21]" /><span>{trek.groupSize}</span>
                  </div>
                  <div className="flex items-center gap-3 text-gray-600">
                    <MapPin className="w-4 h-4 text-[#ffaf21]" /><span className="truncate">{trek.location}</span>
                  </div>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-[#ffaf21] font-medium text-xs">{form.pkg}</span>
                  </div>
                  <hr className="border-gray-100" />
                  <div className="flex justify-between items-center">
                  ...
                  <div className="flex justify-between items-center pt-1">
                    ...
                    <span className="font-bold text-[#ffaf21]">₹{payableNow.toLocaleString()}</span>
                  </div>
                </div>
              </div>

              <div className="bg-[#000000] rounded-2xl p-5 text-white text-sm space-y-3">
                <div className="flex items-center gap-2"><Shield className="w-4 h-4 text-[#ffaf21]" /><span className="font-semibold text-xs">Secure Booking</span></div>
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                ...
                <a href="tel:+919999999999" className="flex items-center gap-3 text-sm text-[#000000] hover:text-[#ffaf21] transition-colors mb-3">
                  <Phone className="w-4 h-4 text-[#ffaf21]" /> +91 99999 99999
                </a>
                <Link href="/contact" className="text-xs text-[#ffaf21] font-medium hover:text-[#d49400] transition-colors">Contact Support &rarr;</Link>
              </div>
            </div>
          </div>
        </form>
...
export default function BookingPage() {
  return (
    <Suspense fallback={
      <div className="pt-28 min-h-screen flex items-center justify-center">
        <Loader className="w-8 h-8 animate-spin text-[#ffaf21]" />
      </div>
    }>
      <BookingContent />
    </Suspense>
