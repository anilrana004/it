...
export default function TrendingPage() {
  ...
  return (
    <div className="pt-24 lg:pt-28 pb-12 lg:pb-20">
      {/* Hero */}
      <section className="bg-gradient-to-r from-[#1a1a2e] to-[#16213e] py-10 lg:py-16 mb-8 lg:mb-12">
        <div className="container mx-auto text-center">
          <div className="flex items-center justify-center gap-3 mb-3">
            <Flame className="w-6 h-6 lg:w-8 lg:h-8 text-[#EA5939]" />
        ...
        <div className="flex gap-2 overflow-x-auto scrollbar-none pb-2 -mx-4 px-4 lg:mx-0 lg:px-0 mb-6">
          {filters.map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={`shrink-0 px-5 py-1.5 rounded-full text-xs lg:text-sm font-medium transition-all ${filter===f?'bg-[#1a1a2e] text-white':'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>{f}</button>
          ))}
        </div>
          ...
          {filtered.map(t => (
            <Link key={t.title} href={t.href} className="group bg-white rounded-xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-lg transition-all">
              <div className="relative h-40 lg:h-44 overflow-hidden">
                ...
                <div className="absolute top-2 right-2 bg-white/90 backdrop-blur-sm text-[10px] font-bold px-2 py-1 rounded-full text-[#EA5939] flex items-center gap-1">
              ...
              <div className="p-3 lg:p-4">
                ...
                <div className="flex items-center">
                  <span className="text-[#359DFC] font-bold text-sm lg:text-base">₹{t.price.toLocaleString()}</span>
                  <span className="ml-auto bg-blue-50 text-[#359DFC] text-[10px] font-semibold px-2 py-0.5 rounded-full">Trending</span>
                </div>
              </div>
            </Link>
          ))}
