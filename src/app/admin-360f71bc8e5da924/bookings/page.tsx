  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-[family-name:var(--font-heading)] text-2xl font-bold text-[#1a1a2e]">Bookings</h1>
          <p className="text-gray-500 text-sm">{b.length} total bookings</p>
        </div>
        <div className="relative">
          <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
          <input type="text" placeholder="Search..." value={search} onChange={e => setSearch(e.target.value)}
            className="pl-9 pr-4 py-2 rounded-xl border border-gray-200 text-sm outline-none focus:border-[#359DFC] w-48 lg:w-64" />
        </div>
      </div>
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                <tr key={book.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                  ...
                  <td className="p-4"><span className="text-xs font-semibold text-[#359DFC]">{book.package}</span></td>
