  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-[family-name:var(--font-heading)] text-2xl lg:text-3xl font-bold text-[#1a1a2e]">Dashboard</h1>
          <p className="text-gray-500 text-sm">Welcome to TrekRoot Admin Panel</p>
        </div>
        <Link href="/" className="text-sm text-[#359DFC] hover:underline hidden lg:block">View Site &rarr;</Link>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-8">
        {stats.map(s => (
          <Link key={s.label} href={s.href} className="bg-white rounded-2xl p-4 lg:p-5 shadow-sm border border-gray-100 hover:shadow-md transition-all">
            ...
            <p className="text-2xl lg:text-3xl font-bold text-[#1a1a2e]">{s.value}</p>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 lg:p-6 mb-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-bold text-lg text-[#1a1a2e]">Recent Bookings</h2>
          <Link href="/admin-360f71bc8e5da924/bookings" className="text-sm text-[#359DFC] hover:underline flex items-center gap-1">View All <ArrowRight className="w-3 h-3" /></Link>
        </div>
        {bookings.length === 0 ? (
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 lg:p-6">
        <h2 className="font-bold text-lg text-[#1a1a2e] mb-4">Quick Actions</h2>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            ...
            <Link key={a.l} href={a.h} className="flex items-center gap-3 p-3 lg:p-4 bg-gray-50 rounded-xl hover:bg-[#359DFC]/5 hover:border-[#359DFC]/20 border border-transparent transition-all">
              <div className="w-9 h-9 bg-[#359DFC]/10 rounded-lg flex items-center justify-center"><a.icon className="w-4 h-4 text-[#359DFC]" /></div>
