              ...
              <tbody>
                {filtered.length === 0 ? (
                  ...
                  <tr key={msg.id} onClick={() => markRead(msg)} className="border-b border-gray-50 hover:bg-gray-50/50 cursor-pointer">
                    <td className="p-4 font-medium text-gray-800 flex items-center gap-2">{msg.status === 'new' && <span className="w-2 h-2 bg-[#359DFC] rounded-full shrink-0" />}{msg.name}</td>
        ...
        {selected && (
          <div className="lg:col-span-1 bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold text-[#1a1a2e]">Message Detail</h3>
            ...
            <div className="mt-4 flex gap-2">
              {selected.status === 'new' && (
                <button onClick={() => { updateContactStatus(selected.id, 'read'); setC([...contacts]); }} className="text-xs font-semibold px-3 py-1.5 bg-[#359DFC] text-white rounded-full hover:bg-[#1a7de0]">Mark Read</button>
              )}
