...
export default function AdminSidebar() {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside className={`fixed lg:sticky top-0 left-0 z-40 h-screen w-64 bg-[#000000] text-white transition-transform duration-300 ${open ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="p-5 border-b border-white/10">
          <Link href="/admin-360f71bc8e5da924" className="flex items-center gap-2">
            <div className="w-8 h-8 bg-gradient-to-br from-[#16a34a] to-[#16a34a] rounded-lg flex items-center justify-center">
          ...
          {sidebarItems.map(item => {
            ...
            return (
              <Link key={item.href} href={item.href} onClick={() => setOpen(false)}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all ${active ? 'bg-[#16a34a] text-white font-semibold' : 'text-gray-300 hover:bg-white/5'}`}>
