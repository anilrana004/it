    ...
    <section className="py-10 lg:py-16">
      <div className="container mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h2 className="font-[family-name:var(--font-heading)] text-2xl lg:text-3xl font-bold text-[#040921]">Sacred Yatras</h2>
            <p className="text-gray-500 text-sm mt-1">Spiritual journeys to Himalayan shrines</p>
          </div>
          <Link href="/yatra" className="text-sm font-medium text-[#afde1e] hover:text-[#8cb818] hidden lg:inline-flex items-center gap-1">View All Yatras <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg></Link>
        </div>
          ...
          {yatras.map(t => (
            <Link key={t.id} href={`/yatra/${t.id}`} className="group block bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-xl transition-all duration-300">
              <div className="relative overflow-hidden aspect-[3/4]">
                ...
                <div className="absolute top-3 left-3"><span className="text-[11px] font-bold px-2.5 py-1 rounded-full bg-[#afde1e] text-gray-900">Yatra</span></div>
              ...
              <div className="p-4 lg:p-5">
                <h3 className="font-bold text-sm lg:text-base text-gray-900 leading-snug line-clamp-2 group-hover:text-[#afde1e] transition-colors mb-2">{t.title}</h3>
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-xs font-medium text-[#afde1e] bg-[#afde1e]/5 px-2 py-0.5 rounded-full">{t.bestSeason}</span>
                </div>
