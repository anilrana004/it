    ...
    <section className="py-10 lg:py-16">
      <div className="container mx-auto">
        <div className="flex items-center justify-between mb-8">
          ...
          <Link href="/yatra" className="text-sm font-medium text-[#16a34a] hover:text-[#15803d] hidden lg:inline-flex items-center gap-1">View All Yatras <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg></Link>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 lg:gap-6">
          {yatras.map(t => (
            <Link key={t.id} href={`/yatra/${t.id}`} className="group block rounded-2xl overflow-hidden transition-all duration-300 relative aspect-[3/4]">
              ...
              <div className="absolute top-3 left-3"><span className="text-[11px] font-bold px-2.5 py-1 rounded-full bg-[#16a34a] text-white">Yatra</span></div>
              ...
              <div className="absolute bottom-0 left-0 right-0 p-4 lg:p-5">
                <h3 className="font-bold text-sm lg:text-base text-white leading-snug line-clamp-2 group-hover:text-[#16a34a] transition-colors mb-2">{t.title}</h3>
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-xs font-medium text-[#16a34a] bg-black/20 backdrop-blur-sm px-2 py-0.5 rounded-full">{t.bestSeason}</span>
                </div>
                <div className="flex items-center justify-between mt-3">
                  <span className="font-bold text-lg text-[#16a34a]">₹ {Math.min(...t.pricing.map(p=>p.price)).toLocaleString()}</span>
