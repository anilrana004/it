...
export default function Hero() {
  ...
  /* ======================== MOBILE LAYOUT ======================== */
  const Mobile = () => (
    <section className="bg-gradient-to-b from-[#fafafa] to-white">
      <div className="container mx-auto px-4 pb-3">

        {/* 1. Search button */}
        <div className="pt-3">
          <button type="button" onClick={() => setShowSearch(true)}
            className="w-full flex items-center gap-3 bg-white border border-gray-200 rounded-2xl px-5 py-4 shadow-sm hover:shadow-md hover:border-[#ffaf21] transition-all active:scale-[0.98]">
            <div className="w-10 h-10 rounded-full bg-[#ffaf21]/10 flex items-center justify-center shrink-0">
              <Search className="w-5 h-5 text-[#ffaf21]" />
            </div>
            <div className="flex-1 min-w-0 text-left">
              <div className="text-sm font-semibold text-gray-900">Where to?</div>
              <div className="text-xs text-gray-400 truncate">Destinations · Treks · Yatras</div>
            </div>
            <div className="text-xs text-gray-300 font-medium bg-gray-50 px-3 py-1.5 rounded-full shrink-0">Search</div>
          </button>
        </div>
        ...
        <div className="flex items-center gap-2 mt-3">
          <Link href="/reviews"
            className="flex items-center gap-1.5 bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200/60 rounded-xl px-3 py-2 active:scale-95 transition-transform shrink-0">
            <div className="flex">
              {[1, 2, 3, 4, 5].map(i => <Star key={i} className="w-3 h-3.5 fill-[#ffaf21] text-[#ffaf21]" />)}
            </div>
            <span className="text-sm font-bold text-gray-800">4.8</span>
            <span className="text-[11px] text-gray-400 font-medium">(10k+)</span>
          </Link>
