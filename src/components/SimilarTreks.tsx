...
export default function SimilarTreks({ currentId, type }: { currentId: string; type: 'trek' | 'yatra' }) {
  ...
  const accent = type === 'yatra' ? '#ffaf21' : '#ffaf21';
        ...
        {similar.map(t => {
          ...
          return (
            <Link key={t.id} href={`/${type === 'yatra' ? 'yatra' : 'treks'}/${t.id}`} className="group bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-lg transition-all">
              ...
              <div className="p-4">
                <h3 className="font-bold text-sm lg:text-base text-[#000000] group-hover:text-[#ffaf21] transition-colors line-clamp-1">{t.title}</h3>
                ...
                <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-100">
                  <div><span className="text-lg font-bold text-[#000000]">₹{minPrice.toLocaleString()}</span><span className="text-xs text-gray-400 ml-1">/person</span></div>
