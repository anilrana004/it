...
export default function SimilarTreks({ currentId, type }: { currentId: string; type: 'trek' | 'yatra' }) {
  ...
  const accent = type === 'yatra' ? '#EA5939' : '#359DFC';

  return (
    <section className="mt-16 lg:mt-20">
      <div className="text-center mb-8">
        <h2 className="font-[family-name:var(--font-heading)] text-2xl lg:text-3xl font-bold text-[#1a1a2e] mb-2">Similar {type === 'yatra' ? 'Yatras' : 'Treks'}</h2>
            ...
            <Link key={t.id} href={`/${type === 'yatra' ? 'yatra' : 'treks'}/${t.id}`} className="group bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-lg transition-all">
              <div className="relative h-48 overflow-hidden">
                ...
                <div className="absolute bottom-3 left-3 right-3">
                  <span className="text-xs font-bold px-2 py-1 rounded-full text-white" style={{ backgroundColor: accent }}>{type === 'yatra' ? 'Yatra' : 'Trek'}</span>
                </div>
              </div>
              <div className="p-4">
                <h3 className="font-bold text-sm lg:text-base text-[#1a1a2e] group-hover:text-[#359DFC] transition-colors line-clamp-1">{t.title}</h3>
                ...
                <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-100">
                  <div><span className="text-lg font-bold text-[#1a1a2e]">₹{minPrice.toLocaleString()}</span><span className="text-xs text-gray-400 ml-1">/person</span></div>
