    ...
    <section className="py-8 lg:py-16 bg-gray-50">
      <div className="container mx-auto">
        <div className="text-center mb-6 lg:mb-8">
          <p className="text-[#afde1e] font-semibold text-xs lg:text-sm tracking-widest uppercase mb-1">CUSTOMISED TOURS</p>
          <h2 className="text-xl lg:text-3xl font-bold text-[#040921]">Get a Customised Tour Package</h2>
        </div>
        <div className="flex justify-center gap-2 mb-6">
          {['International', 'India'].map(t => (
            <button key={t} onClick={() => setTab(t)}
              className={`px-5 py-1.5 rounded-full text-xs lg:text-sm font-medium transition-all ${tab===t?'bg-[#040921] text-white':'bg-white text-gray-600 border border-gray-200 hover:border-gray-300'}`}>{t}</button>
          ))}
        </div>
            ...
            {items.map(d => (
              <Link key={d.name} href={d.href}
                ...
                <div className="relative h-[280px] lg:h-[320px] overflow-hidden">
                  ...
                  <div className="absolute top-3 left-3 bg-[#afde1e] text-gray-900 text-xs font-bold px-2.5 py-1 rounded-full">
                    {d.count}+ Packages
                  </div>
                  <div className="absolute bottom-0 left-0 right-0 p-4">
                    <h3 className="font-bold text-lg text-white group-hover:text-[#afde1e] transition-colors">{d.name}</h3>
        ...
        <div className="text-center mt-6">
          <Link href="/customized" className="text-[#afde1e] text-sm font-semibold hover:text-[#8cb818] transition-colors">View All Customized Tours &rarr;</Link>
        </div>
      </div>
    </section>
  );
