...
import GearRentalPremiumHero from '@/components/rental/GearRentalPremiumHero';
      ...
      <div className="it-rental__card-body">
        ...
        <ul className="it-rental__card-specs" aria-label={`${item.name} details`}>
          <li>
            ...
            {item.rentedLastMonth.toLocaleString('en-IN')} rented last season
          </li>
          {item.sizes?.length ? (
            <li>
...
const RENT_VS_BUY = [
  { id: 'trek-shoes', rent: 800, buy: 5000 },
  { id: 'down-jacket', rent: 500, buy: 3500 },
  { id: 'rain-jacket', rent: 400, buy: 1800 },
  { id: 'rucksack', rent: 700, buy: 4500 },
  { id: 'trek-poles', rent: 200, buy: 2500 },
  { id: 'headlamp', rent: 200, buy: 800 },
  { id: 'sunglasses', rent: 180, buy: 900 },
  { id: 'poncho', rent: 250, buy: 1200 },
];
...
export default function GearRentalPageView({ initialTrekId }: Props) {
  ...
  const [needTrek, setNeedTrek] = useState(false);
  ...
  const rentTotal = RENT_VS_BUY.reduce((sum, row) => sum + row.rent, 0);
  const buyTotal = RENT_VS_BUY.reduce((sum, row) => sum + row.buy, 0);
  const activeCategory = GEAR_CATEGORIES.find((chip) => chip.id === category);
  const cartItemCount = trekLines.reduce((n, line) => n + line.qty, 0);

  const openPicker = (item: GearItem) => {
    setNeedTrek(false);
    setPicking(item);
  };
  ...
  return (
    <article className="it-prep it-prep--gear-rental it-rental">
      <GearRentalPremiumHero />

      <div className="it-prep__wrap it-prep__wrap--solo">
        <PrepTocNav
          items={GEAR_PAGE_TOC.map((item) => ({
            id: item.id,
            title: item.title,
            ...('live' in item && item.live ? { live: true as const } : {}),
          }))}
        />

        <div>
          <section id="why-rent" className="it-prep__section">
            <h2 className="it-prep__section-title">Why rent with Indian Treks</h2>
            <p className="it-prep__section-lead">
              Skip buying gear you will use once a year. We maintain, clean, and stock sizes at every
              major base camp — you just show up and trek.
            </p>
            <div className="it-prep__grid">
              {GEAR_VALUE_PROPS.map((prop) => (
                <div key={prop.title} className="it-prep__grid-card">
                  <h3>
                    <i className={`fa-solid ${prop.icon}`} aria-hidden />
                    {prop.title}
                  </h3>
                  <p>{prop.body}</p>
                </div>
          ...
          <section id="rent-or-buy" className="it-rental__compare-band">
            <div className="it-rental__compare-inner">
              ...
              <p className="it-rental__compare-toggle-hint">
                {compare === 'rent'
                  ? 'Item-wise rental prices for a typical departure'
                  : 'What the same kit costs if you purchase it new'}
              </p>
          ...
          <section id="how-renting-works" className="it-prep__section">
            <h2 className="it-prep__section-title">How renting works</h2>
            <p className="it-prep__section-lead">
              Book online. Collect at base. Travel light.
            </p>
          ...
          <section id="rental-store" className="it-rental__store-band">
            <div className="it-rental__store-inner">
              <div className="it-rental__section-head it-rental__section-head--store">
                ...
                <h2>Rent trail-ready gear for your departure</h2>
                <span className="it-rental__store-kicker-note">
                  Pick sizes, add to your booking, collect at base camp
                </span>
              </div>
              ...
              <div className="it-rental__store-toolbar">
                <div className="it-rental__trekbar" id="rental-trek">
                  <div className="it-rental__trekbar-head">
                    ...
                    {selectedTrek && cartItemCount > 0 ? (
                      <span className="it-rental__trekbar-pill">
                        {cartItemCount} in cart · {inr(rentalTotal)}
                      </span>
                    ) : null}
                  </div>
                  <select
                  ...
                  {needTrek ? (
                    <p className="it-rental__trek-hint is-error" role="status">
                      Choose your departure first so we reserve kit at the right base camp.
                    </p>
                  ) : selectedTrek ? (
                    ...
                    <p className="it-rental__trek-hint">
                      Gear is reserved against an Indian Treks departure — pick your trek first.
                    </p>
                  )}
                </div>
              ...
              <div className="it-rental__store-filters">
                <div className="it-rental__store-filter-head">
                  ...
                  {!trekId ? (
                    <span className="it-rental__store-filter-note">Select a departure to rent</span>
                  ) : null}
                </div>
              ...
              {!trekId ? (
                <p className="it-rental__store-empty">
                  Tap <strong>Rent</strong> on any item — you can pick your departure in the next step.
                </p>
              ) : null}
            </div>
          </section>

          <section id="pickup-points" className="it-prep__section">
            <h2 className="it-prep__section-title">Pickup points</h2>
            <p className="it-prep__section-lead">
              Collect your kit at the base-camp store on day 1 of your departure.
            </p>
            <ul className="it-prep__list">
              {GEAR_STORES.map((store) => (
                <li key={store.name}>
                  ...
                  <span>
                    <strong>{store.name}</strong> — {store.detail}
                  </span>
                </li>
              ))}
            </ul>
          ...
          <section id="rental-policies" className="it-prep__section">
            <h2 className="it-prep__section-title">Policies & FAQs</h2>
            <p className="it-prep__section-lead">
              Deposits, size changes, damage, and when to book — the essentials before you travel.
            </p>
