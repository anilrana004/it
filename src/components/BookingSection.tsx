    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-center gap-2 mb-8">
        {['Trip Details', 'Contact Info', 'Confirm'].map((s, i) => (
          <div key={s} className="flex items-center gap-2">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${step > i+1 ? 'bg-[#ffaf21] text-gray-900' : step === i+1 ? 'bg-[#ffaf21] text-gray-900' : 'bg-gray-100 text-gray-400'}`}>
              {step > i+1 ? <Check className="w-4 h-4" /> : i+1}
            </div>
            <span className={`text-xs font-medium hidden sm:inline ${step === i+1 ? 'text-[#ffaf21]' : 'text-gray-400'}`}>{s}</span>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 lg:p-8">
            ...
            <div className="grid grid-cols-3 gap-3">
              {trek.pricing.map(p => (
                <button key={p.name} type="button" onClick={() => setForm(f => ({...f, pkg: p.name}))}
                  className={`p-4 rounded-xl border-2 text-center transition-all ${form.pkg===p.name ? 'border-[#ffaf21] bg-[#ffaf21]/5' : 'border-gray-100 hover:border-gray-200'}`}>
                  <div className="font-bold text-sm text-[#000000]">{p.name}</div>
                  <div className="text-lg font-bold text-[#ffaf21]">₹{p.price.toLocaleString()}</div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input type="date" required value={form.date} onChange={e => setForm(f => ({...f, date: e.target.value}))}
                  className="w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:border-[#ffaf21] focus:ring-2 focus:ring-[#ffaf21]/20 outline-none transition-all" />
              </div>
              <div>
              ...
              <div className="flex gap-3">
                {[
                  ...
                  <button key={o.v} type="button" onClick={() => setForm(f => ({...f, payment: o.v}))}
                    className={`flex-1 p-3 rounded-xl border-2 text-center transition-all ${form.payment===o.v ? 'border-[#ffaf21] bg-[#ffaf21]/5' : 'border-gray-100 hover:border-gray-200'}`}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input id="booking-name" type="text" required value={form.name} onChange={e => setForm(f => ({...f, name: e.target.value}))} aria-invalid={!!errors.name} aria-describedby={errors.name ? 'err-name' : undefined}
                  className={`w-full px-4 py-2.5 rounded-lg border outline-none transition-all focus:border-[#ffaf21] focus:ring-2 focus:ring-[#ffaf21]/20 ${errors.name ? 'border-red-400' : 'border-gray-200'}`} placeholder="Your full name" />
              ...
              <div>
                ...
                <input id="booking-email" type="email" required value={form.email} onChange={e => setForm(f => ({...f, email: e.target.value}))} aria-invalid={!!errors.email} aria-describedby={errors.email ? 'err-email' : undefined}
                  className={`w-full px-4 py-2.5 rounded-lg border outline-none transition-all focus:border-[#ffaf21] focus:ring-2 focus:ring-[#ffaf21]/20 ${errors.email ? 'border-red-400' : 'border-gray-200'}`} placeholder="email@example.com" />
              ...
              <div>
                ...
                <input id="booking-phone" type="tel" required value={form.phone} onChange={e => setForm(f => ({...f, phone: e.target.value}))} aria-invalid={!!errors.phone} aria-describedby={errors.phone ? 'err-phone' : undefined}
                  className={`w-full px-4 py-2.5 rounded-lg border outline-none transition-all focus:border-[#ffaf21] focus:ring-2 focus:ring-[#ffaf21]/20 ${errors.phone ? 'border-red-400' : 'border-gray-200'}`} placeholder="+91 98765 43210" />
              ...
              <div>
                ...
                <input id="booking-notes" type="text" value={form.notes} onChange={e => setForm(f => ({...f, notes: e.target.value}))}
                  className="w-full px-4 py-2.5 rounded-lg border border-gray-200 outline-none transition-all focus:border-[#ffaf21] focus:ring-2 focus:ring-[#ffaf21]/20" placeholder="Dietary needs, health conditions..." />
              </div>
            </div>

            <div className="bg-[#ffaf21]/5 rounded-xl p-4 flex items-start gap-3">
        ...
        <div className="flex gap-3 mt-8">
          ...
          <button type="submit" disabled={loading} className="flex-1 flex items-center justify-center gap-2 font-semibold px-6 py-3 rounded-full transition-all text-sm bg-[#ffaf21] hover:bg-[#d49400] text-gray-900 disabled:opacity-50 disabled:cursor-not-allowed">
